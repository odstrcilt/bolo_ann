...
def get_data(shot):
    ...
    #EFIT_data = np.load("EFIT_data.npy") 
    #BOLO_data = np.load("BOLO_data.npy")     
    ...
    return tvec, EFIT_data, norm_power,BOLO_data_norm 
...
def load_mds_data(shot, EFIT = 'EFIT01', realtime_bolo=False):
    ...
    return tvec[tind], inputs.T[tind], bolo_brightness.T[tind]   # tvec, EFIT_data, BOLO_data
...
def apply_model(tvec,EFIT_data,norm_power,BOLO_data_norm):


    module_dir = os.path.dirname(os.path.abspath(__file__))
    h5_path = os.path.join(module_dir, "tomo_nn_model.h5")
    print("loading from ", h5_path)
    with h5py.File(h5_path, "r") as f:          # TODO improve path
        # Load params
    ...
    # -----------------------
    device_type = "cpu"
    jax_device = jax.devices(device_type)[0]
    #gpu = jax.devices("gpu")[0]
    BOLO_data_norm = BOLO_data_norm.astype(np.float32)
    print(" types BOLO, EFIT ", BOLO_data_norm.dtype, EFIT_data.dtype)
    print(" BOLO, EFIT ", BOLO_data_norm.shape, EFIT_data.shape)
    
    jit_apply = jax.jit(model.apply)
    with jax.default_device(jax_device):
    
        _ = jit_apply(params, BOLO_data_norm, EFIT_data).block_until_ready()
        ts = time.time()
        #y_pred = model.apply(params, BOLO_data_norm, EFIT_data)
        y_pred = jit_apply(params, BOLO_data_norm, EFIT_data).block_until_ready()
        y_pred *= norm_power[:,None]  # denormalize
    print(" pred time "+ device_type,time.time() - ts)
...
def main():
    ...
    shot = 175860
    print(shot)
    # tvec, EFIT_data, BOLO_data
    tvec, EFIT_data, norm_power,BOLO_data_norm = get_data(shot)
    
    reconstructed = apply_model(tvec,EFIT_data,norm_power,BOLO_data_norm)
    plot(reconstructed)
