...
model = NMF(n_components= N_OUTPUT_BASIS, init="nndsvda", max_iter=100)
W = model.fit_transform(tomo)   # (n, k)
B_init = model.components_         # (k, m)
B_init /= B_init.max(1)[:,None]
#inverse of softplus
B_init = np.log(np.expm1(B_init + 0.01))
...
class TomoNet(nn.Module):
    ...
    def __call__(self, raw, efit):
        ...
        y_pos = jax.nn.softplus(y)
        B_pos = jax.nn.softplus(self.B)
        img = jnp.dot(y, B_pos)
...
with h5py.File("tomo_nn_model_nnf.h5", "w") as f:
